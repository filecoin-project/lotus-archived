...
import (
	...
	"context"
	"encoding/binary"
	store2 "github.com/filecoin-project/lotus/chain/store"
...
func UpgradeActorsV2(ctx context.Context, sm *StateManager, cb ExecCallback, root cid.Cid, ts *types.TipSet) (cid.Cid, error) {
	//cbs := cachebs.NewBufferedBstore(sm.cs.Blockstore(), 1_000_000)
	buf := bufbstore.NewTieredBstore(sm.cs.Blockstore(), bstore.NewTemporarySync())
	store := store2.ActorStore(ctx, buf)
...

	{
		...
		if err := vm.Copy(ctx, from, to, root); err != nil {
