...
func GetNetworkName(ctx context.Context, sm *StateManager, st cid.Cid) (dtypes.NetworkName, error) {
	...
	ias, err := init_.Load(sm.cs.StateStore(ctx), act)
	if err != nil {
		return "", err
	}
...
func GetMinerWorkerRaw(ctx context.Context, sm *StateManager, st cid.Cid, maddr address.Address) (address.Address, error) {
	...
	mas, err := miner.Load(sm.cs.StateStore(ctx), act)
	if err != nil {
	...
	return vm.ResolveToKeyAddr(state, sm.cs.StateStore(ctx), info.Worker)
}
...
func GetPowerRaw(ctx context.Context, sm *StateManager, st cid.Cid, maddr address.Address) (power.Claim, power.Claim, bool, error) {
	...
	pas, err := power.Load(sm.cs.StateStore(ctx), act)
	if err != nil {
...
func GetMinerSlashed(ctx context.Context, sm *StateManager, ts *types.TipSet, maddr address.Address) (bool, error) {
	...
	spas, err := power.Load(sm.cs.StateStore(ctx), act)
	if err != nil {
...
func GetStorageDeal(ctx context.Context, sm *StateManager, dealID abi.DealID, ts *types.TipSet) (*api.MarketDeal, error) {
	...
	state, err := market.Load(sm.cs.StateStore(ctx), act)
	if err != nil {
...
func ListMinerActors(ctx context.Context, sm *StateManager, ts *types.TipSet) ([]address.Address, error) {
	...
	powState, err := power.Load(sm.cs.StateStore(ctx), act)
	if err != nil {
...
func minerHasMinPower(ctx context.Context, sm *StateManager, addr address.Address, ts *types.TipSet) (bool, error) {
	...
	ps, err := power.Load(sm.cs.StateStore(ctx), pact)
	if err != nil {
		return false, err
	}
...
func MinerEligibleToMine(ctx context.Context, sm *StateManager, addr address.Address, baseTs *types.TipSet, lookbackTs *types.TipSet) (bool, error) {
	...
	pstate, err := power.Load(sm.cs.StateStore(ctx), pact)
	if err != nil {
		return false, err
	}
	...
	mstate, err := miner.Load(sm.cs.StateStore(ctx), mact)
	if err != nil {
		return false, err
	}
...
func CheckTotalFIL(ctx context.Context, sm *StateManager, ts *types.TipSet) (abi.TokenAmount, error) {
	str, err := state.LoadStateTree(sm.ChainStore().StateStore(ctx), ts.ParentState())
	if err != nil {
