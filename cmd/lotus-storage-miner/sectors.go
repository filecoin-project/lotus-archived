...
var sectorsListCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		fullApi, closer2, err := cliutil.GetFullNodeAPI(cctx) // TODO: consider storing full node address in config
		if err != nil {
			return err
		}
		defer closer2()
...
var sectorsTerminatePendingCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		defer closer()
		api, nCloser, err := cliutil.GetFullNodeAPI(cctx)
		if err != nil {
			return err
		}
		defer nCloser()
...
var sectorsCapacityCollateralCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		mApi, mCloser, err := cliutil.GetStorageMinerAPI(cctx)
		if err != nil {
			return err
		}
		defer mCloser()

		nApi, nCloser, err := cliutil.GetFullNodeAPI(cctx)
		if err != nil {
			return err
		}
		defer nCloser()
